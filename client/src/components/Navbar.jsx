...
const Navbar = ({ user: userProp }) => {
  ...
  return (
    <>
    {/* 🔴 بداية التعديل: صندوق ثابت يجمع الناف بار وشريط الأذكار */}
    <div className="fixed top-0 left-0 right-0 z-50 flex flex-col w-full">
      
      {/* تم استبدال sticky هنا واستخدام الستايل من الصندوق الأب */}
      <nav className={`app-header border-b bg-white/90 dark:bg-[#0f1720]/90 backdrop-blur-md shadow-sm transition-colors duration-300 ${navLoading ? 'loading' : ''}`} >
      ...
      {/* تم نقل شريط الأذكار هنا داخل الـ Fixed Container ليتحرك مع الناف بار */}
      <AthkarBar />
    </div> 
    {/* 🔴 نهاية التعديل: إغلاق الصندوق الثابت */}

    {/* عنصر وهمي (Spacer) لتعويض الارتفاع حتى لا يختفي المحتوى خلف البار الثابت */}
